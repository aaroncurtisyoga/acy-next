...
const EventCard: FC<EventCardProps> = ({ event: initialEvent }) => {
  ...
  return (
    <div className="w-full mb-4">
      {/* Edit Badge - positioned above card on the right */}
      {isAdmin && !isEditing && (
        <div className="flex justify-end pr-[30px] -mb-[1px]">
          <button
            onClick={() => setIsEditing(true)}
            className="px-3 py-1 text-xs text-foreground-600 hover:text-foreground-800 bg-white dark:bg-gray-800 border border-divider rounded-t-lg border-b-0 transition-colors duration-200"
          >
            Edit
          </button>
        </div>
      )}
      <Card className="w-full shadow-small hover:shadow-medium transition-shadow duration-200 rounded-3xl @container">
