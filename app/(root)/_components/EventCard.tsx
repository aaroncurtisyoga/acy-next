...
const EventCard: FC<EventCardProps> = ({ event, isHighlighted = false }) => {
  ...
  return (
    <div className="w-full mb-4">
      <Card
        id={`event-${event.id}`}
        className={`w-full border ${isHighlighted ? "border-2 border-primary bg-blue-50/50 shadow-lg" : "border-border"} shadow-none hover:shadow-sm transition-shadow duration-200 rounded-2xl @container`}
      >
